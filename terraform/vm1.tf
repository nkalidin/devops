...
provider "azurerm" {
  ...
  subscription_id = "xxxx-xxxx-xxxx-xxxx-xxxx-xxxx"
...
resource "azurerm_resource_group" "samosa" {
  name     = "samosa"
  location = "east us"
  tags = {
    "name" = "samosa1"
...
resource "azurerm_subnet" "hexagon-net1-s2" {
  name                 = "hexagon-net1-s2"
  resource_group_name  = azurerm_resource_group.vadapav.name
  virtual_network_name = azurerm_virtual_network.hexagon-net1.name
  address_prefixes     = ["90.0.0.128/25"]
}
